...
const DeterminismIcon = ({ active }: { active: boolean }) => (
  <svg viewBox="0 0 60 60" className="w-full h-full">
    {/* Cache layers */}
    <motion.rect
      x="10" y="10" width="40" height="40" rx="4"
      fill="none" stroke="currentColor" strokeWidth="2"
      opacity={0.3}
    />
    <motion.rect
      x="15" y="15" width="30" height="30" rx="3"
      fill="currentColor"
      opacity={0.2}
      animate={{ opacity: active ? [0.2, 0.4, 0.2] : 0.2 }}
      transition={{ duration: 2, repeat: active ? Infinity : 0 }}
    />
    {/* Hash symbols */}
    <motion.text
      x="30" y="35" textAnchor="middle"
      fill="currentColor" fontSize="16" fontFamily="monospace" fontWeight="bold"
...
const tiers = [
  {
    ...
    innovations: [
      ...
      },
      {
        ...
        description: "SSOT is compiled into canon.json. CI blocks merge if drift detected. Not documentation—enforced law.",
        formula: "services.truth.yml → canon.json → CI gate → ledger event",
      },
      {
  ...
  },
  {
    id: "tier6",
    tier: "TIER 6",
    title: "Court-Grade Determinism",
    subtitle: "Same Input = Same Output = Same Hash",
    color: "#10b981",
    Icon: DeterminismIcon,
    description: "Not just caching—mathematical proof of determinism. Every operation is auditable and reproducible.",
    innovations: [
      {
        name: "Hash Contract",
        proof: "Q.E.D.",
        description: "Every 'money endpoint' has input_hash, context_hash, output_hash. Same inputs = same hashes = cache hit.",
        formula: "cache_key = f(op, rbac, context_hash, input_hash)",
      },
      {
        name: "Fail-Open Integrity",
        proof: "Protocol",
        description: "If Redis/Ledger fails, business logic continues but flags degraded state. Auditor sees truth, not hidden failure.",
        formula: "_integrity_level: 'nominal' | 'degraded' | 'emergency'",
      },
      {
        name: "Ledger Events",
        proof: "Audit Trail",
        description: "Every operation emits {op}.start → {op}.finish events. Complete audit trail for court validation.",
        formula: "ledger.emit('{op}.start', {input_hash, context_hash})",
      },
    ],
  },
];
    ...
    <main className="min-h-screen bg-void pt-14">
      ...
      <section className="py-24 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          >
            <p className="text-xs font-mono text-text-ghost tracking-[0.3em] uppercase mb-4">
              6 Tiers of Innovation
            </p>
            <h2 
              className="text-3xl md:text-4xl text-text-bright"
              style={{ fontFamily: "var(--font-instrument-serif)" }}
            >
              From Math Proofs to Court-Grade Determinism
            </h2>
          </motion.div>
      ...
      {/* SPICE Deep Dive */}
      <section id="spice" className="py-32 px-6 bg-surface-1/20">
        <div className="max-w-5xl mx-auto">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
